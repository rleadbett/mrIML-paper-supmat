...
generateM_specific_type <- function(
  nn,
  k_ave,
  type.network = "random",
  ...
  if (type.interact == "random") {
    for (i in 1:nl) {
      A[edgelist[i, 1], edgelist[i, 2]] <- runif(
        1,
        min = -interact.str.max,
        max = interact.str.max
      )
      A[edgelist[i, 2], edgelist[i, 1]] <- runif(
        1,
        min = -interact.str.max,
        max = interact.str.max
      )
